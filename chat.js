...
composerEl.addEventListener("submit", async (e) => {
  ...
  // --- Chatbot commands (private, not sent to the public room)
  // `/teach <text>` -> adds to shared knowledge base
  const teachMatch = text.match(/^\/teach\s+([\s\S]+)/i);
  if (teachMatch) {
    const lesson = String(teachMatch[1] || "").trim().slice(0, 1200);
    if (lesson.length < 8) {
      alert("Teach text is too short.");
      return;
    }

    const stickToBottom = isNearBottom(messagesEl);
    addMessage(
      { id: `local-${Date.now()}-${Math.random().toString(36).slice(2)}`, username: user, text: lesson, ts: Date.now() },
      { stickToBottom }
    );

    try {
      const teachRef = push(ref(database, "bot_knowledge/items"));
      await set(teachRef, { text: lesson, author: user, ts: serverTimestamp() });
      addMessage(
        {
          id: `local-bot-${Date.now()}-${Math.random().toString(36).slice(2)}`,
          username: "FCBot",
          text: "Learned. Thanks! (Saved to shared knowledge.)",
          ts: Date.now(),
        },
        { stickToBottom: true }
      );
      ...
      addMessage(
        {
          id: `local-bot-${Date.now()}-${Math.random().toString(36).slice(2)}`,
          username: "FCBot",
          text: "I couldn't save that right now (Firebase error).",
          ts: Date.now(),
        },
        { stickToBottom: true }
      );
  ...
  // `/bot <message>` or `@FCBot <message>` -> requests AI response (via backend function)
  const botMatch = text.match(/^\/(?:bot|ai)\s+([\s\S]+)/i) || text.match(/^@FCBot\s+([\s\S]+)/i);
  if (botMatch) {
    const prompt = String(botMatch[1] || "").trim().slice(0, 800);
    if (!prompt) return;

    const stickToBottom = isNearBottom(messagesEl);
    addMessage(
      { id: `local-${Date.now()}-${Math.random().toString(36).slice(2)}`, username: user, text: prompt, ts: Date.now() },
      { stickToBottom }
    );

    const thinkingRow = addMessage(
      {
        id: `local-bot-thinking-${Date.now()}-${Math.random().toString(36).slice(2)}`,
        username: "FCBot",
        text: "Thinking…",
        ts: Date.now(),
      },
    ...
    try {
      const reqRef = push(ref(database, "bot_requests"));
      const reqId = reqRef.key;
      await set(reqRef, { prompt, author: user, ts: serverTimestamp(), status: "queued" });

      const respRef = ref(database, `bot_requests/${reqId}/response`);
      const unsub = onValue(respRef, (snap) => {
        if (!snap.exists()) return;
        const v = snap.val() || {};
        const text = typeof v.text === "string" ? v.text : "";
        const body = thinkingRow?.querySelector?.(".msg__body");
        if (body) body.textContent = text || "No response.";
        unsub();
        // Optional cleanup: keep the request for logs; remove if desired.
      });

      // Timeout guard: if no response arrives, stop listening.
      window.setTimeout(() => {
        try {
          unsub();
        } catch {
        ...
        if (body && body.textContent === "Thinking…") {
          body.textContent = "No response yet. (Bot backend not deployed?)";
        }
      }, 12000);
    } catch (err) {
      const body = thinkingRow?.querySelector?.(".msg__body");
      if (body) body.textContent = `Bot error: ${err?.message || String(err)}`;
    }
